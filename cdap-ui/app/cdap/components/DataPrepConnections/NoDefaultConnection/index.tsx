...
const NoDefaultConnection: React.SFC<INoDefaultConnectionProps> = ({ defaultConnection, showAddConnectionPopover}) => {
  if (
    isNilOrEmpty(defaultConnection) ||
    (defaultConnection && isNilOrEmpty(defaultConnection.name))
  ) {
    return (
      <EmptyMessageContainer title="No default connection available">
        <span>
          <br />
          <ul>
            <li>
  ...
  const { name: connectionId } = defaultConnection;
  const {type} = defaultConnection;
